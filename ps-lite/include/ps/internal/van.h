...
//==================== xym edit to record the communication trace ==================//
  /**
   * \brief used to record the trace during the network interation -- xym 2017-12-11
   */
  virtual void Trace_Send(const Message& msg, int send_bytes)=0;

  /**
   * \brief used to record the trace during the network interation -- xym 2017-12-11
   */
  virtual void Trace_Recv(Message *msg, size_t send_bytes) = 0;  
  /**
  * \brief used to record the trace during the network interation -- xym 2017-12-29
  */
  virtual void Trace_Send_(const Message& msg, int send_bytes, int key, struct timeval start)=0;

  /**
  * \brief used to record the trace during the network interation -- xym 2017-12-29
  */
  virtual void Trace_Recv_(Message *msg, size_t send_bytes, int key, struct timeval end) = 0;
//==================== xym edit to record the communication trace =================//
