...
class KVWorker : public SimpleApp {
 public:
  ...
  int Push(const std::vector<Key>& keys,
    ...
    return ZPush(
        SArray<Key>(keys), SArray<Val>(vals), SArray<int>(lens), cmd, cb, -10);//xym add this 2017-12-11   -10代表push的默认值
  }
  ...
  int Pull(const std::vector<Key>& keys,
           ...
           int cmd = 0,
           const Callback& cb = nullptr) {
    return Pull_(SArray<Key>(keys), vals, lens, cmd, cb, -5);
  }
  ...
  int ZPush(const SArray<Key>& keys,
            ...
            int cmd = 0,
            const Callback& cb = nullptr,
	    int zpush_count = -4) { //-4代表默认值，用于检测worker读取的值是否为默认值;
    ...
    kvs.vals = vals;
    kvs.lens = lens;
    Send(ts, true, cmd, kvs, zpush_count); //xym edit this 2017-12-10
    return ts;
  }
  ...
  int ZPull(const SArray<Key>& keys,
            ...
            const Callback& cb = nullptr,
	    int zpull_count = -4) {
    return Pull_(keys, vals, lens, cmd, cb, zpull_count);
  }
 ...
 private:
  ...
  int Pull_(const SArray<Key>& keys, C* vals, D* lens,
            int cmd, const Callback& cb, int pull_count); //xym edit this 2017-12-10
  /**
   ...
   * @param cmd command
   */
  void Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs, int send_count); //send_count edited by xym 2017-12-10
...
struct KVMeta {
  ...
  /** \brief used to record the push_pull_operation from the server*/
  int push_pull_operation; //xym edit this 2017-12-10
  int recv_key; // xym edit this 2018-01-02;
};
...
struct KVServerDefaultHandle {
  void operator()(
    ...
    KVPairs<Val> res;
    if (req_meta.push) {
      res.keys = req_data.keys; // xym edit this to deliver the keys;
...
void KVServer<Val>::Process(const Message& msg) {
  ...
  meta.push_pull_operation = msg.meta.push_pull_operation; // xym edit this 2017-12-11
  meta.recv_key  = msg.meta.recv_key; // xym edit this 2018-01-02
  KVPairs<Val> data;
  int n = msg.data.size();
  if (n) {
    CHECK_GE(n, 2);
...
void KVServer<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
  ...
  msg.meta.push_pull_operation = req.push_pull_operation; // xym edit this 2017-12-11
  msg.meta.recv_key    = req.recv_key; // xym edit this 2018-01-02
  if (res.keys.size()) {
...
template <typename Val>
void KVWorker<Val>::Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs, int send_count) {
  // slice the message
  SlicedKVs sliced;
...
//======================= xym add this to get back the keys -- 2018-01-02 ===================//

  int key_xu = kvs.keys[0];
/*
  FILE *file = fopen("/home/hadoop/tensor/data_trace/lost_pull_check.txt","a");
  fprintf(file,"KVWorker::Send_Key = %d\n",key_xu);
  fclose(file);
*/
//======================= xym add this to get back the keys -- 2018-01-02 ===================//
  for (size_t i = 0; i < sliced.size(); ++i) {
    ...
    msg.meta.push_pull_operation = send_count; // xym edit this 2017-12-11
    msg.meta.recv_key    = key_xu; // xym add this 2018-01-02
...
int KVWorker<Val>::Pull_(
    const SArray<Key>& keys, C* vals, D* lens, int cmd, const Callback& cb, int pull_count) {
  int ts = obj_->NewRequest(kServerGroup); //pull_count used to record the pull operation  -- xym edit 2017-12-11
  ...
  Send(ts, false, cmd, kvs, pull_count); // xym edit this 2017-12-11
  return ts;
}
