...
//                                                                                                                     //
//******************************************** xym edit 2017-12-25 ****************************************************//
    void Trace_Send_(const Message& msg, int send_bytes, int key, struct timeval end) override {
        std::lock_guard<std::mutex> lk(xu_);//避免不同线程的同时写
//        gettimeofday(&end, NULL);
        int send_id;
        int recv_id;
        ...
        if (Postoffice::Get()->is_worker()) {
            if(msg.meta.recver == 1){
                ...
                recv_id = (msg.meta.recver - 8) / 2 + num_workers;//编号最开始是worker，之后是server，最后是scheduler；此处接收节点为server节点
 		        recv_id_t = recv_id - num_workers;
                if(msg.meta.push){
                    //此处为Push_Send, push的值默认是false，所以如果push的值为true，必然不会是最初的初始化通信过程，只需要在Pull中对初始化操作进行分析即可；
                    if(Init_Flag_Wor[recv_id_t][key] == 0){
			    ...
			    fprintf(file,"=====================================================================================================================================\n");
                            ...
                            fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Push_Send_Worker\t\t%d-%d-s%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                    record_count, send_id, recv_id, send_bytes, msg.meta.push_pull_operation, key, Trace_Push_Send[recv_id_t][key].operation_num, recv_id_t, 0, 0, end.tv_sec, end.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                            fclose(file);
                        }
                        else{
                            FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
                        ...
                        Init_Flag_Wor[recv_id_t][key] = 1;//经过一次操作之后，falg的值置为1，在下一次执行同一个key值的push_send操作时，必然存在依赖；
                    }//end Init_Flag == 0;
                    else{
				...
				fprintf(file,"%d-s%d,", Trace_Pull_Recv[sj][key].operation_num, sj);
			fprintf(file,")\n");
	                fclose(file);
                    }
                }//end msg.meta.push
			    ...
			    fprintf(file,"=====================================================================================================================================\n");                            
			    	...
			    	fprintf(file,"=====================================================================================================================================\n"); 
                                ...
                                fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Pull_Send_Worker\t\t%d-%d-s%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                        record_count, send_id, recv_id, send_bytes, msg.meta.push_pull_operation, key, Trace_Pull_Send[recv_id_t][key].operation_num, recv_id_t, 0, 0, end.tv_sec, end.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                                fclose(file);
                            }
                            else{
                                FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
...
//========================================= This is about the Server Operation 2017-12-25 ==========================================//
            ...
            if(msg.meta.recver == 1){
			    ...
			    fprintf(file,"=====================================================================================================================================\n");
                            ...
                            fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Push_Send_Server\t\t%d-%d-w%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                    record_count, send_id, recv_id, send_bytes, msg.meta.push_pull_operation, key, Ser_Push_Send[recv_id][key].operation_num, recv_id, 0, 0, end.tv_sec, end.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                            fclose(file);
                        }
                        else{
                            FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
			    ...
			    fprintf(file,"%d-%d-w%d\n", key, Ser_Push_Recv[recv_id][key].operation_num,recv_id);
			    fclose(file);
			}
			else{
			    fprintf(file,"(");
                            for(int i = 0; i < num_workers; ++i)
                                fprintf(file,"%d-%d-w%d, ", key, Ser_Push_Recv[i][key].operation_num, i);
                        	fprintf(file, ")\n");
                        	fclose(file);
                        //============异步模式下的依赖关系===========//
                        /*fprintf(file,"w%d_%d-%d\n",recv_id, Ser_Push_Recv[recv_id][key].operation_num, key);
                         fclose(file);*/
			}
                    }
                }//meta.Push_send
			    ...
			    fprintf(file,"=====================================================================================================================================\n");
			    	...
			    	fprintf(file,"=====================================================================================================================================\n");
                                ...
                                fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Pull_Send_Server\t\t%d-%d-w%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                        record_count, send_id, recv_id, send_bytes, msg.meta.push_pull_operation, key, Ser_Pull_Send[recv_id][key].operation_num, recv_id, 0, 0, end.tv_sec, end.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                                fclose(file);
                            }
                            else{
                                FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
    ...
    void Trace_Recv_(Message* msg, size_t send_bytes, int key, struct timeval start) override {
        //      std::lock_guard<std::mutex> lk(xu_);//避免不同线程的同时写
//        gettimeofday(&start,NULL);
        int send_id;//记录发送的节点
	int send_id_t;
...
//======================================= this is about the worker operation =========================================//
        if(Postoffice::Get()->is_worker()){
            ...
            if(msg->meta.sender%2 != 0){
                ...
                send_id = (msg->meta.sender - 8)/2 + num_workers;//此处发送的节点是server节点；
		send_id_t = send_id - num_workers;
                if(msg->meta.push){
                    if(Init_Flag_Wor[send_id_t][key] == 0){
			    ...
			    fprintf(file,"=====================================================================================================================================\n");
                            ...
                            fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Push_Recv_Worker\t\t%d-%d-s%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                    record_count, send_id, recv_id, send_bytes, msg->meta.push_pull_operation, key, Trace_Push_Recv[send_id_t][key].operation_num, send_id_t, 0, 0, start.tv_sec, start.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                            fclose(file);
                        }
                        else{
                            FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
                        ...
                        Init_Flag_Wor[send_id_t][key] = 1;//经过一次操作之后，falg的值置为1，在下一次执行同一个key值的push_send操作时，必然存在依赖；
                    }//end Init_Flag == 0;
                    else{
			   	...
			   	fprintf(file,"==           |==  push_recv_worker <---- push_send_server\n ");
			    	...
			    	fprintf(file,"=====================================================================================================================================\n");
                                ...
                                fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Pull_Recv_Worker\t\t%d-%d-s%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                        record_count, send_id, recv_id, send_bytes, msg->meta.push_pull_operation, key, Trace_Pull_Recv[send_id_t][key].operation_num,send_id_t,0, 0, start.tv_sec, start.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                                fclose(file);
                            }
                            else{
                                FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
...
//======================================= this is about the server operation ========================================//
        if(Postoffice::Get()->is_server()) {
			...
			if(msg->meta.push_pull_operation == -15)
			   Ser_Push_Recv[send_id][key].operation_num = -2;
			else
			   ...
			   fprintf(file,"=====================================================================================================================================\n");
                           ...
                           fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Push_Recv_Server\t\t%d-%d-w%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                    record_count, send_id, recv_id, send_bytes, msg->meta.push_pull_operation, key, Ser_Push_Recv[send_id][key].operation_num, send_id, 0, 0, start.tv_sec, start.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                            fclose(file);
                        }
                        else{
                            FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_record.txt", "a");
			   	...
			   	fprintf(file,"\tServer_ID: =");
			    	...
			    	fprintf(file,"=====================================================================================================================================\n");
                                ...
                                fprintf(file, "%d\t%d\t%d\t%d\t\t%d\t\tOP:= Pull_Recv_Server\t\t%d-%d-w%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
                                        record_count, send_id, recv_id, send_bytes, msg->meta.push_pull_operation, key, Ser_Pull_Recv[send_id][key].operation_num, send_id, 0, 0, start.tv_sec, start.tv_usec, -1);//如果record_count值为0，必然不存在依赖，此时的依赖为-1，就表示不存在依赖；
                                fclose(file);
                            }
                            else{
...
//=====================================================================================================================//
  ...
  int SendMsg(const Message& msg) override {
    ...
    gettimeofday(&end,NULL);// get the time at the start of push operation
    // find the socket
...
//      Trace_Send(msg, send_bytes);
      Trace_Send_(msg, send_bytes, msg.meta.recv_key, end);
//====================xym edit this 12-11 ========================//
      return send_bytes;
  }
...
//    Trace_Recv(msg, recv_bytes);
    gettimeofday(&start, NULL);
    Trace_Recv_(msg, recv_bytes, msg->meta.recv_key, start);
//===================== xym edit-12-11 =================//
    return recv_bytes;
  }
...
//用于记录worker在分布式训练过程中的trace
    std::unordered_map<int, Trace_> Trace_Push_Send[5];
    std::unordered_map<int, Trace_> Trace_Push_Recv[5];
    std::unordered_map<int, Trace_> Trace_Pull_Send[5];
    std::unordered_map<int, Trace_> Trace_Pull_Recv[5];
    std::unordered_map<int, int> Init_Flag_Wor[5];//在写入文件时，如果key值对应的Flag值为0，则不存在依赖，否则都有对同一可以值其他操作的依赖；
//用于记录server在分布式训练过程中的trace
    std::unordered_map<int, Trace_> Ser_Push_Recv[5]; //此处2的值为分布式执行过程中的worker_num值，
    std::unordered_map<int, Trace_> Ser_Push_Send[5];
    std::unordered_map<int, Trace_> Ser_Pull_Recv[5];
    std::unordered_map<int, Trace_> Ser_Pull_Send[5];
    std::unordered_map<int, int> Init_Flag_Ser[5];
//**************************** xym edit ****************************//
