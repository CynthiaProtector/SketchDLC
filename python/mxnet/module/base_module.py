    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                if monitor is not None:
                    monitor.tic()
#================== xym add this -2018-01-05 =======================#
# tic1, tic2, tic3 are added by xym 
		tic1 = time.time()
                self.forward_backward(data_batch)
		tic2 = time.time()
                self.update()
		tic3 = time.time()
		f = open("/home/hadoop/tensor/data_trace/batch_data.txt","a")
		print >>f,"======Batch_Num: = %d\t" %(Batch_Num),"Forward_backward_time: = %.8f,\tupdate_time: = %.8f\t\t"%((tic2-tic1),(tic3-tic2)), data_batch
		f.close()
		tic4 = time.time()
                try:
		...
		Batch_Num += 1
            ...
            toc = time.time()
            self.logger.info('Epoch[%d] Time cost=%.3f', epoch, (toc-tic-(tic4-tic3)))
