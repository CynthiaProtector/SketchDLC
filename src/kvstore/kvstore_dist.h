...
//========================= xym add this -- 2018-01-03 ========================//
/*    FILE *file = fopen("/home/hadoop/tensor/check_SendCommandToServer.txt","a");
    fprintf(file,"Using the SendCommandToServers function\n");
    fclose(file);
*/
//========================= xym add this -- 2018-01-03 ========================//
 ...
 private:
  ...
  void InitImpl(const std::vector<int>& keys,
    ...
    if (get_rank() == 0) {
      Push_(keys, values, 0, false, -15); // xym edit -2017-12-11 -15 represent the init operation
...
//    Push_(keys, values, priority, true); // the orginal version of code
     dependency_count++;
/*
     FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_pp.txt","a");
     fprintf(file,"push_pull_count:= %d\t\tOP: push_operation\n", dependency_count);
     fclose(file);
*/
//==========================xym edit 2017-12-11===============================//
     Push_(keys, values, priority, true, dependency_count); // xym edit this 2017-12-11
  }
...
//=====================xym edit this 2017-12-11 ============================//
    dependency_count++;
    int pullimpl = 0;
    pullimpl = dependency_count;
/*
    FILE *file = fopen("/home/hadoop/tensor/data_trace/trace_pp.txt","a");
    fprintf(file,"push_pull_count:= %d\t\tOP: pull_operation\n",dependency_count);
    fclose(file);
*/
//=====================xym edit this 2017-12-11 ============================//
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      auto pull_from_servers = [this, key, recv_buf, pullimpl](
        ...
        CHECK_NOTNULL(ps_worker_)->ZPull(
          pskv.keys, vals, &pskv.lens, cmd, [vals, cb](){ delete vals; cb(); }, pullimpl); // xym add this 2017-12-11
      };
  ...
  void Push_(const std::vector<int>& keys,
             ...
             int priority,
             bool do_merge,
	     int push_count) {//xym add this 2017-12-11;
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      if (storage_type == kDefaultStorage) {
        if (gradient_compression_->get_type() == CompressionType::kNone) {
          ...
          PushDefault(key, comm_buf, pskv, priority, push_count); // xym add this -- 2017-12-11
        } else {
          ...
          if (is_active) {
            PushCompressed(key, comm_buf, pskv, priority, push_count);// xym add this push_count --2017-12-11
          } else {
            PushDefault(key, comm_buf, pskv, priority, push_count); // xym add this push_count --2017-12-11
          }
        }
        ...
        PushRowSparse(key, comm_buf, priority, push_count);  // xym add this push_count -- 2017-12-11
      } else {
  ...
  void PushCompressed(int key, const NDArray& comm_buf, const PSKV& pskv, int priority, int push_count) {
    auto &small_buf = compr_buf_[key];// xym add this -- 2017-12-11
    ...
    auto push_to_servers =
      [this, key, pskv, small_buf, push_count](RunContext rctx, Engine::CallbackOnComplete cb) { //xym add the push_count 2017-12-11;
        ...
        CHECK_NOTNULL(ps_worker_)->ZPush(
          ...
          static_cast<int>(DataHandleType::kCompressedPushPull), [cb]() { cb(); }, push_count);//xym add the push_count 2017-12-11
      };
  ...
  void PushDefault(int key, const NDArray &send_buf, const PSKV& pskv, int priority, int push_count) {
    auto push_to_servers =
        [this, key, pskv, send_buf, push_count](RunContext rctx, Engine::CallbackOnComplete cb) {
	  // xym add the push_count --2017-12-11
          // convert to ps keys
          ...
          CHECK_NOTNULL(ps_worker_)->ZPush(
              pskv.keys, vals, pskv.lens,
              static_cast<int>(DataHandleType::kDefaultPushPull), [cb]() { cb(); }, push_count);// xym add this -- 2017-12-11
        };
    Engine::Get()->PushAsync(
        push_to_servers,
        pinned_ctx_,
  ...
  void PushRowSparse(int key, const NDArray &send_buf, int priority, int push_count) {
    using namespace rowsparse;// xym add the push_count above -- 2017-12-11
    auto push_to_servers = [this, key, send_buf, push_count] 
                           (RunContext rctx, Engine::CallbackOnComplete cb) {// xym add this -- 2017-12-11
#if MKL_EXPERIMENTAL == 1
      mkl_set_tblob_eager_mode(send_buf.data());
#endif
      real_t* data = send_buf.data().dptr<real_t>();
  ...
  void PullRowSparse_(const int key, const NDArray& recv_buf,
    ...
    auto pull_from_servers = [this, key, recv_buf, indices]
      ...
      CHECK_NOTNULL(ps_worker_)->ZPull(pskv.keys, vals, &pskv.lens,
                                       static_cast<int>(DataHandleType::kRowSparsePushPull),
                                       [vals, cb]() { delete vals; cb(); }, dependency_count);// xym add this -- 2017-12-11
    };
    CHECK_NOTNULL(Engine::Get())->PushAsync(
      pull_from_servers,
      pinned_ctx_,
  ...
  int dependency_count = 0; //used to count the push and pull operation -- xym add 2017-12-11
